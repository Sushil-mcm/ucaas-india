...
| E2 | `mod_callcenter` fails to load — config template's not-found path returns an empty string, :9002 serves callcenter.conf as 400 (tracker F1). User-run script `backend-patches/freeswitch/apply-callcenter-load.sh` exists | After the user runs the script: verify the load, then fix the template so `<agents>` and `<tiers>` render (tracker F2 — today only `<queues>`) | fs-xml-api callcenter template; FreeSWITCH modules config | Queue/agent/tier rows already stored | callcenter.conf returns 200 with agents+tiers present; `show application` lists callcenter; a test call to a queue rings a joined agent |
