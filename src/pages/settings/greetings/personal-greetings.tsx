...
import { COMPANY_LOCK_WORDING, type GreetingSlot } from '@/lib/company-policy';
import { greetingOptionsForSlots } from '@/lib/greeting-slots';
import { LiveFlag } from '../not-applied-note';

const PersonalGreetings = ({
  customClass,
  /* Which slots the company rule has locked for this person; the page reads
     the rule once and passes the answer down. */
  isSlotLocked = () => false,
}: {
  customClass?: string;
  isSlotLocked?: (slot: GreetingSlot) => boolean;
}) => {
  ...
  const mediaOptionsGreetingNotifications = [
    ...
    ...option,
    lockNote: isSlotLocked(option.name as GreetingSlot) ? COMPANY_LOCK_WORDING : undefined,
  }));
