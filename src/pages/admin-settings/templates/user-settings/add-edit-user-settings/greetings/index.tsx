...
const GreetingNotification: FC<IGREETINGPROPS> = ({ intro, footer, containerClass }: any) => {
  ...
  return (
    ...
    >
      {intro}
      {/* Active since 9 Sep 2026: the switch reads these four slots on every
          call that rings a person or a group (dialplan_service.py,
          call_audio_actions and person_greeting_actions). The person's own
          choice wins; the company's applies where the person has none, or
          always when the company row is locked. Queues keep the audio set on
          the queue itself. Keep this note true if that changes. */}
      <SettingCard
        ...
        status="active"
        note="Active on calls to a person or a group: the welcome message plays before ringing, the ringback tone plays while they ring, the on-hold music plays when they put the caller on hold, and the voicemail message plays before the beep. A person's own choice on their Greetings tab wins over the company's unless the company has locked it. Queues play the recordings chosen on the queue itself. A new choice reaches the switch within a minute."
      >
