 ...
 *
 * UPDATED 19 Sep 2026 — a person form now honours these, as the old note here
 * asked whoever did it to record.
 *
 * WHAT IS LIVE: People › (a person) › Profile fields renders every definition
 * below and saves the answers onto that person, under the same key, via
 * `src/lib/person-profile-fields.ts` — a pure, tested payload builder rather
 * than logic inside a screen, because `/api/user/update` replaces the whole
 * record and this codebase has erased voicemail greetings that way before.
 *
 * WHAT IS STILL NOT: no directory column shows these, no API accepts them from
 * anywhere else, and a person cannot edit their own. That is the owner's
 * decision of 19 Sep, not an oversight — admin-only avoids publishing staff
 * data. Keep this note accurate if that changes.
 *
        ...
        <div className="flex w-full flex-col gap-4">
          ...
          <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
            ...
            <div className="flex flex-col gap-4 p-4">
              {/* Still the honest bit, but the honest thing changed on 19 Sep:
                  there IS now somewhere to fill these in. What remains true is
                  that they are administrator-only — no directory column, and a
                  person cannot edit their own. An admin who sets a field up
                  should know exactly where it turns up, and where it does not. */}
              <p className="rounded-lg border border-sky-200 bg-sky-50 px-3 py-2 text-xs text-sky-900">
                What you define here appears on each person&apos;s record, under{' '}
                <strong>People &rsaquo; (a person) &rsaquo; Profile fields</strong>, where an
                administrator fills the answers in. They are not shown in the company directory or
                the people list, and a person cannot edit their own.
              </p>
