...
const QueueSettings: FC<any> = ({ scriptList, setModalState }) => {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4 overflow-y-auto pr-1">
      ...
      {/* Agent screen capture (10 Sep 2026). The softphone reads this off the
          queue record when a queue call reaches an agent, records the agent's
          shared screen with MediaRecorder for the length of the call, uploads
          the .webm beside the audio recording and links it (tenant-api
          screen_recordings). The agent shares their screen once per shift -
          a browser will only grant that on a click - and the app asks them to
          when this is on. No share means the call goes on unrecorded. */}
      <SettingCard
        title="Agent screen capture"
        description="Record what is on the agent's screen during this queue's calls, alongside the audio."
        status="active"
        note="Lives in the app: the agent is asked to share their screen for the shift, and each call on this queue is recorded from answer to hang-up. Supervisors open it from the call record. Kept as long as call recordings are."
      >
        <SettingRow
          label="Also record the agent's screen"
          description="Off — audio only, as before. On — the agent's screen is recorded on every call from this queue they answer while sharing."
          control={
            <Switch
              className="cursor-pointer"
              checked={watch('settings.screen_capture') === true}
              onCheckedChange={(checked: boolean) =>
                setValue('settings.screen_capture', checked, { shouldValidate: true })
              }
            />
          }
        />
