...
import { getMonitoringLiveCalls, isActiveMonitoringCall } from '@/pages/monitoring/live-call-helpers';
...
/* The two cells that show live data (presence, on-call state) read the socket
   feeds themselves and are memoised, so a socket tick re-renders only these
   cells — not the whole 200-row table. Everything else in `columns` closes over
   stable values, which keeps the columns array (and the memoised TableManager)
   from rebuilding on every tick. */
const AvailabilityCell = memo(({ person }: { person: any }) => {
  ...
  const live = getAgentLiveState(person?.extension, usersOnlineStatus, activeQueueCalls);
...
type ActionsCellProps = {
  person: any;
  pendingInviteByUuid: Map<string, { expired: boolean }>;
  onOpenDrawer: (key: string) => void;
  onSelectUser: (user: any) => void;
  onDelete: (open: boolean) => void;
  onResendInvite: (payload: { user_uuid: string }) => void;
...
const ActionsCell = memo(
  ({
    ...
    const isMeOnCall = usersOnlineStatus?.find((u: any) => u?.userId == extension)?.onCall;

    const isLoggedInUsers = user?.uuid === data?.uuid;
    ...
    const actions = [
      ...
      virtualNumbersAccess?.assign_number && {
        icon: 'PlusIcon',
        onClick: () => {
          onOpenDrawer('assignUser');
          onSelectUser({ ...data, user_uuid: data?.user_uuid || data?.uuid });
        },
      ...
      userAccess?.add &&
        pendingInviteByUuid.has(data?.uuid) && {
          ...
          onClick: () => onResendInvite({ user_uuid: data?.uuid }),
...
const UsersExtension: FC = () => {
  ...
  const pendingInviteByUuid = useMemo(
    () =>
      new Map<string, { expired: boolean }>(
  ...
  const columns: ColumnDef<IUSERS>[] = useMemo(
    () => [
    {
      header: 'Name',
    ...
    },
    {
      ...
      cell: ({ row }) => <AvailabilityCell person={row?.original} />,
    },
    {
      header: 'Actions',
      accessorKey: 'action',
      cell: ({ row }) => (
        <ActionsCell
          person={row?.original}
          pendingInviteByUuid={pendingInviteByUuid}
          onOpenDrawer={openDrawer}
          onSelectUser={setSelectedUser}
          onDelete={setOpen}
          onResendInvite={mutateResendInvite}
    ...
    [IS_ADMIN, pendingInviteByUuid, openDrawer, mutateResendInvite, isResendingInvite],
  );
