...
import { callInboundList, forwardActionType } from '@/services/api';
...
import { FORWARD_TYPES_ARR, handleStatus, STATUS_TYPE } from '../constant';
...
const Inbound = () => {
  ...
  const [activeTab, setActiveTab] = useState('Total Calls');
  ...
  const tabs = [
    ...
    { label: 'Answered ', count: callStats?.inbound_calls || 0 },
    { label: 'Missed ', count: callStats?.missed_calls || 0 },
    { label: 'Voicemails', count: callStats?.voicemail || 0 },
    { label: 'Total Duration', count: callStats?.total_duration || 0 },
  ];
    ...
    <ReportsPageLayout filters={Filters}>
      <div className="w-full  p-3 flex flex-col gap-2">
        <div className="flex gap-3 overflow-x-auto pb-1 sm:grid sm:grid-cols-3 sm:overflow-visible sm:pb-0 md:grid-cols-5">
          {tabs.map((tab) => (
            <div
              key={tab.label}
              onClick={() => {
                if (tab.label !== 'Total Duration') handleTabClick(tab.label);
              }}
        ...
        <TableManager
          {...{
            tableRef,
            fetcherKey: 'inboundCallList',
            fetcherFn: callInboundList,
            select: (data: any) => data?.data?.data?.result?.data?.data,
            onSuccess: (data: any) => {
              setCallStats(data?.data?.data?.result?.data?.call_stats || null);
            },
            columns,
            search,
