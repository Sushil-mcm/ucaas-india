...
export const TaskIcon = ({ className }: IconProps) => {
  /* This never took a `className` before, so every caller's sizing
     (the sidebar rail asks for `h-[1.15rem] w-[1.15rem]`, matching every
     other rail icon) was silently dropped and the svg stayed pinned at its
     raw 12x12. `currentColor` also brings it in line with the rest of the
     set, which recolour on hover/active because their icon tracks the
     button's own text colour instead of a fixed navy. */
  return (
    <svg
      width="12"
      height="12"
    ...
    >
      <g>
        <path d="M10 3L1.5 3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
        <path d="M5 5.5L1.5 5.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
        <path d="M5 8H1.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
        <path
...
// Kept for ucaas.in: icon.tsx/type.ts still register the Zapier glyph (unified dropped it with its Zapier page).
