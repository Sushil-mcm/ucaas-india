...
export default class JitsiStreamBackgroundEffect {
   ...
   */
  constructor(model: any, options: IBackgroundEffectOptions) {
    this._options = options;

    if (this._options.virtualBackground.backgroundType === VIRTUAL_BACKGROUND_TYPE.IMAGE) {
      this._virtualImage = document.createElement('img');
      this._virtualImage.crossOrigin = 'anonymous';
      console.log('Loading virtual background image from:', this._options.virtualBackground.virtualSource);
      this._virtualImage.src = this._options.virtualBackground.virtualSource ?? '';

      // Add error handling for image loading
      this._virtualImage.onerror = (error) => {
        console.error('Failed to load virtual background image:', error);
        console.error('Image source:', this._virtualImage.src);
      };
  ...
  runPostProcessing() {
    ...
    this._outputCanvasCtx.filter =
      backgroundType === VIRTUAL_BACKGROUND_TYPE.IMAGE ? 'blur(4px)' : 'blur(8px)';
    ...
    if (backgroundType === VIRTUAL_BACKGROUND_TYPE.IMAGE) {
      this._outputCanvasCtx?.drawImage(
        // @ts-ignore
        backgroundType === VIRTUAL_BACKGROUND_TYPE.IMAGE ? this._virtualImage : this._virtualVideo,
        0,
        0,
