 ...
 *      Buying numbers, changing the plan, paying the bill, adding an office.
 ...
 *      why the office manager who can do everything else still cannot buy.
 *
 ...
 *      line separates an office administrator from a team manager more cleanly
 ...
 *
 * Run those five over any capability and the six kinds fall out:
 *
 *   admin        the whole company, money included. Usually the person who
 *                signed up.
 *   sub_admin    everything the admin can do except spend money and reshape the
 *                account. The person who actually runs the day to day.
 *   manager      one team: its call routing, its hours, its members, and
 *                watching it work. Cannot create or delete people.
 *   supervisor   watches the team and nothing else. Cannot change a setting.
 *                Promoted from within the team, not sent down from head office.
 *   agent        does the work, sees their own numbers. The tools of the job and
 *                nothing about anybody else.
 *   user         an ordinary person with a phone. Their own line, their own
 *                data, nothing about anybody else.
 *
 * TWO THINGS THIS PRODUCT DOES NOT HAVE YET, SAID PLAINLY
 *
 *   No scope. A permission here applies to the whole company. "Manager" means
 *   the routing of every team, not of theirs, because there is no column
 *   saying which team is theirs. Question 2 above is therefore only half
 *   answerable today, and the manager and supervisor defaults are written as
 *   the narrowest set that is still useful rather than the correct one.
 *   See lib/admin-scope.ts, which records the missing half.
 *
...
/** The six kinds of person this file has an opinion about. */
export type RoleTier = 'admin' | 'sub_admin' | 'manager' | 'supervisor' | 'agent' | 'user';

/** Widest first. Used for ordering on screen and nothing else. */
...
export const TIERS: TierInfo[] = [
  {
    ...
    description:
      'The whole company, money included. Buys numbers, changes the plan, pays the bill, adds offices, and can do everything below.',
  ...
  },
  {
    ...
    description:
      'Runs the day to day: adds and removes people, sets up call routing, connects other software, reads the whole company’s reports.',
    boundary:
      'No spending and no changing the shape of the account — no buying numbers, no plan changes, no billing, no new offices.',
  ...
  },
  {
    ...
    label: 'Team manager',
    description:
      'Runs a team: its call routing, its opening hours, who is in it, and watching it work. Reads reports and listens to recordings.',
    boundary:
  ...
  },
  {
    ...
    label: 'Supervisor',
    description:
      'Watches the team while it works — sees who is on a call, listens in, whispers, takes a call over, and reads the team’s reports.',
    boundary:
      'Changes no settings at all. No routing, no hours, no adding people to the team. Watching, not configuring.',
  ...
  },
  {
    ...
    label: 'Agent',
    description:
      'Takes and makes calls as part of a team, with the tools that go with it — the dialler, campaigns, messaging and the shared inbox.',
    boundary:
      'Sees their own calls and nobody else’s. No live monitoring, no other people’s recordings, no settings.',
...
const ALL: RoleTier[] = ['admin', 'sub_admin', 'manager', 'supervisor', 'agent', 'user'];
const ADMINS: RoleTier[] = ['admin', 'sub_admin'];
const WATCHERS: RoleTier[] = ['admin', 'sub_admin', 'manager', 'supervisor'];
const CONFIGURERS: RoleTier[] = ['admin', 'sub_admin', 'manager'];
const WORKERS: RoleTier[] = ['admin', 'sub_admin', 'manager', 'supervisor', 'agent'];
