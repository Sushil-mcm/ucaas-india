...
/* A section nobody has saved yet is not an error: the server answers 404
   ("No settings saved for section …") and the caller's defaults apply. Every
   other failure is still thrown, so a real outage is not mistaken for an
   empty section. */
export const getSection = async (section: string): Promise<CompanySettingsSection | null> => {
  let response: any;
  try {
    response = await getCompanySettingsSection({ section });
  } catch (error: any) {
    const status = Number(error?.response?.status ?? error?.status);
