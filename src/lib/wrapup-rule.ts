...
/**
 * Whether a wrap-up that runs out of time with no label saved should stay
 * open until one is. "Required" rules do; the two that let the agent go
 * (optional, only when asked) and the one whose whole point is being closed
 * for them (required, forced closed) do not. A ticked label is saved for the
 * agent first, whatever the rule - that decision is the caller's.
 */
export const holdsForLabel = (raw: unknown): boolean => {
  const mode = readWrapupMode(raw);
  return mode === WRAPUP_MODES.MANDATORY || mode === WRAPUP_MODES.MANDATORY_TIMEOUT;
};
